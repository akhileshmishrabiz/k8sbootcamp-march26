...
## Demo 5 — Canary deployment (order-service 90/10)

**Requires Linkerd** (not NetworkPolicy-only). Simulates progressive rollout without changing callers — they still use `order-service:8004`.

### Setup

```bash
chmod +x networking/linkerd/canary/*.sh
./networking/linkerd/canary/setup-canary.sh
```
...
| Resource | Role |
|----------|------|
| `order-service` (stable) | Existing deployment, label `version=stable` |
| `order-service-canary` | Copy with `CANARY=true` env |
| `order-service-stable` / `order-service-canary` | Backend Services |
| `TrafficSplit` | 90% stable / 10% canary on apex `order-service` |

### Generate traffic (must be in-cluster / meshed)
...
Or: `./simulate-traffic-incluster.sh`
...
```bash
kubectl logs -n ecommerce deploy/order-service-canary -c order-service --tail=10
```

### Shift weights (progressive rollout)
...
Weights use milli (`1000m` = 100%).

### Teardown

```bash
./networking/linkerd/canary/teardown-canary.sh
```

Full walkthrough: `exercise.md` Part D.

---

## Demo 6 — Identity (who is calling whom)
...
## Demo 7 — L7 authorization (optional advanced)
...
## Demo 8 — Before vs after (quick compare)
